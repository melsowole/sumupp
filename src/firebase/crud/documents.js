...
import { getQueryData } from "./auxiliary.js";

export async function create(collectionName, newDocObj) {
  try {
...
export async function read(collectionName, idOrRef) {
  try {
    const docRef = await getRef(collectionName, idOrRef);
...
export async function readMany(collectionName, whereStatement, getRef) {
  try {
...
export async function update(collectionName, idOrRef, changes) {
  try {
    const docRef = await getRef(collectionName, idOrRef);
...
export async function getRef(collectionName, idOrRef) {
  let docRef;
